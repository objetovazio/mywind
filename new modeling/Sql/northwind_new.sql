...
CREATE TABLE IF NOT EXISTS `northwind_new`.`orders` (
  ...
  INDEX `fk_orders_order_details1_idx` (`order_details_id` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_orders_order_details1`
    ...
    REFERENCES `northwind_new`.`order_details` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
AUTO_INCREMENT = 82;
